...
for \*\*small\*\* values. \|
\| \*\*Pillai's Trace\*\* \| \$\\sum\_{i=1}\^{s}
\\frac{\\lambda_i}{1+\\lambda_i}\$ \| Most robust to violations of
assumptions (like unequal covariance). \|
\| \*\*Hotelling-Lawley\*\* \| \$\\sum\_{i=1}\^{s} \\lambda_i\$ \|
Similar to Wilks, based on \$tr(\\mathbf{E}\^{-1}\\mathbf{H})\$. \|
\| \*\*Roy's Largest Root\*\* \| \$\\lambda_1\$ \| Most powerful if
group differences are concentrated on a single dimension. \|
